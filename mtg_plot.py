...
def blb_plots(df: pd.DataFrame) -> list:
    figs = []
    figs.append(plot_simple_bar(
        df,
...
def otj_plots(df) -> list:
    figs = []

    df_outlaw_gb = df[df['Key_Outlaw'] == 1][c.list_colour].sum().T
    fig = go.Figure(go.Bar(x=df_outlaw_gb.index, y=df_outlaw_gb))
    fig.update_layout(height=600, width=800, barmode='stack', template='plotly_dark')
    ...
    fig = go.Figure([
        go.Bar(name='Do Crime', x=c.list_colour, y=df_do_crime_gb),
        go.Bar(name='On Crime', x=c.list_colour, y=df_on_crime_gb),
    ])
    fig.update_layout(height=600, width=800, barmode='group', template='plotly_dark')
    ...
    fig = go.Figure([
        go.Bar(name='Creatures Plot', x=c.list_colour, y=df_plot_creatures),
        go.Bar(name='Non-Creatures Plot', x=c.list_colour, y=df_plot_noncreatures),
    ])
    fig.update_layout(height=600, width=800, barmode='group', template='plotly_dark')
...
def mh3_plots(df) -> list:
    list_arche = ['Key_Energy', 'Key_Modified', 'Key_Eldrazi', 'Key_Artifacts']
    df_arche_labels = [k.split('Key_')[1] for k in list_arche]
...
def dft_plots(df: pd.DataFrame) -> list:
    figs = []
    figs.append(plot_simple_bar(
        df,
        x_axes=c.dft_archetype_keys,
        y_col_name='Colour',
        y_stack=c.list_colour,
        title='DFT Archetype Keywords by Colours',
    ))
    if 'Key_Vehicle' in df.columns:
        figs.append(plot_simple_bar(
            df[df['Key_Vehicle'] == 1],
            x_axes=c.list_colour,
            y_col_name='Card Type',
            y_stack=c.list_card_type[:2],
            title='Vehicles by Colour',
        ))
    return figs
